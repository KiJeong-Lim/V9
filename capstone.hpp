...
private:
    CAN can;
    MotorHandler *const motor_handlers_vec_ptr;
    const std::size_t motor_handlers_vec_size;
    CANMessage rx_msg;
public:
    template<std::size_t LEN>
    CANHandler(const PinName &rd, const PinName &td, MotorHandler (*const motor_handlers_vec)[LEN])
        : can(rd, td), motor_handlers_vec_ptr(*motor_handlers_vec), motor_handlers_vec_size(LEN), rx_msg(0)
    {
        rx_msg.len = 6;
    }
